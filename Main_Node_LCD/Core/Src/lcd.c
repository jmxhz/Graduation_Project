...
// 修改后的函数声明
void LCD_ShowChinese(u16 x, u16 y, u8 *c1, u8 *c2, u16 color, u16 bg_color)
{
    u16 row, col;
    u8 left_byte, right_byte;

    for (row = 0; row < 16; row++)
    { // 遍历16行
        if (row < 8)
        {
            left_byte = c1[2 * row]; // 前8行数据来自c1
            right_byte = c1[2 * row + 1];
        }
        else
        {
            u8 adj_row = row - 8; // 后8行数据来自c2
            left_byte = c2[2 * adj_row];
            right_byte = c2[2 * adj_row + 1];
        }
        ...
        for (col = 0; col < 8; col++)
        {
            ...
            { // 高位在前解析
                LCD_Fast_DrawPoint(xpos, ypos, color);
            }
            else
            {
                LCD_Fast_DrawPoint(xpos, ypos, bg_color);
            }
        }
